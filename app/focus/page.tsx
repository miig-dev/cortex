...
export default function FocusPage() {
  const [currentTask, setCurrentTask] = useState('Sélectionnez une tâche à accomplir');
  const [taskHistory, setTaskHistory] = useState<Array<{task: string, timeSpent: number, date: Date}>>([]);

  const handleTimeRecorded = (timeSpent: number) => {
    console.log(`Temps enregistré : ${Math.floor(timeSpent / 60)} minutes pour "${currentTask}"`);
  ...
  const handleNewTask = () => {
    const newTask = prompt('Quelle tâche voulez-vous chronométrer ?', currentTask);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 flex flex-col items-center justify-center p-4">
      ...
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-white mb-2">⏱️ Chronomètre de Tâches</h1>
      ...
      {taskHistory.length > 0 && (
        <div className="mt-8 w-full max-w-4xl">
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl p-6 border border-gray-700">
            <h3 className="text-lg font-bold text-white mb-4">📋 Historique des tâches</h3>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {taskHistory.slice().reverse().map((entry, index) => (
